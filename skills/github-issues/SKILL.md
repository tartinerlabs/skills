...
allowed-tools: Read Bash(gh:*) Bash(glab:*) mcp__github__list_issue_fields mcp__github__issue_read mcp__github__issue_write
model: haiku
effort: medium
...
1. **GitHub MCP server tools**, if such a server is connected. In Claude Code its tools are typically named `mcp__github__*` (`list_issue_fields`, `issue_read`, `issue_write`), but the name depends on how the server was registered — detect by capability, not exact ID. Never assume the server is available.
